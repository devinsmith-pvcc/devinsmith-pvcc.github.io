#Name: Devin Smith
#Prog Purpose: This program reads in a hotel data file, performs calculations, and creates an HTML file for the results

import datetime

############ define rate tuples ############

#            SR  DR  SU
#             0   1   2
ROOM_RATES = (195,250,350)

#           s-tax   occ-tax
#              0      1
TAX_RATES = (0.065,0.1125)
 
########### define files and list ############
infile = "emerald.csv"
outfile = "hotelsalesrep.html"

guest = [] 

############ define program functions ############
def main():
    read_in_guest_file()
    perform_calculations()
    open_out_file()
    create_output_html()
            
def read_in_guest_file():
    guest_data = open(infile, "r")
    guest_in   = guest_data.readlines()
    guest_data.close()

    #### split the data and insert into list called: guest
    for i in guest_in:
        guest.append(i.split(","))
        

def perform_calculations():
    global grandtotal
    grandtotal=0
    
    for i in range(len(guest)):
            room_type = str(guest[i][2])
            num_nights = int(guest[i][3])

            if room_type =="SR":
                subtotal = ROOM_RATES[0] * num_nights
#STUDENTS: COMPLETE THESE elif AND else statements
            elif room_type == "DR":
                subtotal = ROOM_RATES[1] * num_nights

            else:
                subtotal = ROOM_RATES[2] * num_nights
                
#STUDENTS: COMPLETE THESE CALCULATIONS        
            salestax  = subtotal * TAX_RATES[0]
            occupancy = subtotal * TAX_RATES[1]
            total     = subtotal + salestax + occupancy
             
            grandtotal += total
        
#STUDENTS: ADD THE REST OF THE append statements after this one       
            guest[i].append(subtotal)
            guest[i].append(salestax)
            guest[i].append(occupancy)
            guest[i].append(total)



def open_out_file():        
    global f
    f = open(outfile, 'w')
    f.write('<html> <head> <title> Emerald Beach Hotel & Resort </title>\n')
    f.write('<style> td{text-align: right} </style> </head>\n')
    f.write('<body style ="background-color: #583e23; background-image: url(wp-emerald.jpg); color: #73c4be;">\n')
    
def create_output_html():
    global f
    
    moneyf="8,.2f"
    today = str(datetime.datetime.now())
    day_time = today[0:16]

    tr = '<tr><td>'
    endtd = '</td><td>'
    endtr = '</td></tr>\n'



 #STUDENTS: INSERT ALL THE MISSING f.write STATEMENTS HERE
    
    f.write('\n<table border="3" style="background-color: #2a2d10; font-family: arial; margin: auto;">\n')
    f.write('<tr><td colspan = 7>\n')
    f.write('<h2 style="text-align:center;">Emerald Beach Hotel & Resort</h2></td></tr>\n')
    f.write('<tr><td colspan=7>\n')
    f.write('<h3 style="text-align:center;">Sales Report Date/Time: ' + str(day_time) + '</h3></tr>')
    f.write(tr + 'Last' + endtd + 'First' + endtd + 'Num Nights' + endtd + 'Subtotal' + endtd + 'Sales Tax' + endtd + 'Occ. Tax' + endtd + 'Total' + endtr)
    for i in range(len(guest)):
        last = str(guest[i][0])
        first = str(guest[i][1])
        num_nights = str(guest[i][3])
        subtotal = int(guest[i][4])
        salestax = int(guest[i][5])
        occupancy = int(guest[i][6])
        total = int(guest[i][7])
        #Dr. Stevens, figuring out how to get this and the f.write to work together took longer than I expected,
        #but the feeling I got when I mentally had the breakthrough was fantastic. Thank you for the brain workout :)

        f.write(tr + last + endtd + first + endtd + num_nights + endtd + format(subtotal,moneyf) + endtd + format(salestax,moneyf) + endtd + format(occupancy,moneyf) + endtd + format(total,moneyf) + endtr)
    f.write('<tr><td colspan=5></td><td>Grand Total</td><td>' + format(grandtotal,moneyf) + endtr)
    f.write("</body></html>")
    f.close()
    print('Open ' + outfile + ' to view data.')

##call on main program to execute##
main()
