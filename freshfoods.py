...
def perform_calculations():
    ...
    #STUDENTS: calculate other deductions and NET PAY here

        net = pay - fed - state - ssec - medi - retire
    #add to totals
        total_gross += pay
        total_net += net
    ...
    #STUDENTS: append other deductions here:
...
#STUDENTS: create the missing code to print out employee data, one line at a time
    for i in range(num_emps):
        data1 = emp[i] + " " + job[i] + format(gross_pay[i], currency) + tab + format(fed_tax[i], currency) + tab
        data2 = format(state_tax[i], currency) + tab + format(soc_sec[i], currency) + tab + format(medicare[i], currency) + tab
        data3 = format(retirement[i], currency) + tab + format(net_pay[i], currency)
